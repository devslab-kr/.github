...
- 🔢 **[numkey](https://github.com/devslab-kr/numkey)** &nbsp;[![npm](https://img.shields.io/npm/v/%40devslab%2Fnumkey)](https://www.npmjs.com/package/@devslab/numkey) — Numeric input formatting for "it's a string, but it's a number" fields: live thousands grouping with a caret that stays put, leading-zero cleanup, full-width digit normalization (Korean/Japanese IME), automatic right-align + `inputmode`, and a string-first canonical value model (money-safe — never IEEE 754). Opt-in locale separators via `Intl` (`data-numkey-locale="auto"`); deterministic display by default. One CDN script tag with `data-numkey` auto-init covers server-rendered JSP/PHP forms — no build step — plus `NumkeyInput` components for Vue 3 / React controlled inputs. kokey's sibling in the "-key" input family. Live demo at [devslab-kr.github.io/numkey](https://devslab-kr.github.io/numkey/).
