...
- 📏 **[editor-ruler](https://github.com/devslab-kr/editor-ruler)** &nbsp;[![npm](https://img.shields.io/npm/v/%40devslab%2Feditor-ruler)](https://www.npmjs.com/package/@devslab/editor-ruler) — 웹 리치텍스트 에디터를 위한 **Word 스타일 가로 줄자** — Froala · TinyMCE · CKEditor 5 · Quill 등 범용 WYSIWYG 어디에도 없는 그 컨트롤: 좌/우 여백 + 첫 줄 들여쓰기 드래그 핸들(내어쓰기 포함), 런타임 전환 가능한 cm/in/px 눈금, 제스처당 undo 경계 1회, 키보드 접근 가능한 ARIA 슬라이더 핸들, CSS 변수 테마. 에디터 불문 의존성 0 코어(`@devslab/editor-ruler`) + 에디터별 어댑터 — `-froala` 우선(표준 플러그인 + `toggleRuler` 툴바 버튼), Tiptap / CKEditor 5 예정. CDN 스크립트 태그로 바로 사용(`EditorRuler` 전역). 라이브 플레이그라운드: [devslab-kr.github.io/editor-ruler](https://devslab-kr.github.io/editor-ruler/).
