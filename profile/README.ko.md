...
- 🔢 **[numkey](https://github.com/devslab-kr/numkey)** &nbsp;[![npm](https://img.shields.io/npm/v/%40devslab%2Fnumkey)](https://www.npmjs.com/package/@devslab/numkey) — "문자열인데 숫자"인 필드를 위한 숫자 인풋 포맷팅: 커서가 튀지 않는 실시간 천 단위 콤마, 앞자리 0 정리, 전각 숫자 정규화(한글/일본어 IME), 오른쪽 정렬 + `inputmode` 자동 설정, 그리고 문자열 우선 정식 값 모델(금액 안전 — IEEE 754를 거치지 않음). `Intl` 기반 로케일 구분자는 옵트인(`data-numkey-locale="auto"`), 기본은 브라우저 무관 고정 표시. CDN 스크립트 태그 한 줄 + `data-numkey` 자동 초기화로 서버 렌더링 JSP/PHP 폼까지 빌드 도구 없이 커버, Vue 3 / React controlled 인풋용 `NumkeyInput` 컴포넌트 포함. kokey의 형제 — "-key" 인풋 패밀리. 라이브 데모: [devslab-kr.github.io/numkey](https://devslab-kr.github.io/numkey/).
